...
│   └── PreCallBriefTriggerService.swift  # Intraday pre-call briefing catcher: on a new work-calendar meeting during 09:00–17:00, spawns headless `claude` running the derived skill (delivery via Slack `chat.postMessage` + `remctl` for Reminders). Also fires the skill in REMOVED mode when a meeting *disappears* (cancelled/moved) → Slack update; `IntradayBriefGate` gates fire/wait/drop (imminent exemption + started grace), `IntradayDiffClassifier` pairs a move into one reschedule. See docs/INTRADAY-BRIEFINGS.md. On **confirmed Claude exhaustion** it hands over to the opt-in Apple Intelligence fallback (`BriefingFallbackCoordinator`, `BriefingFallbackProviders`, `BriefingNotionRepository`, `BriefingDeliveryService`, `BriefingPartnerResolver`, `BriefingCoordinationLease`) which retrieves, generates, writes and delivers the briefing itself, then lets Claude enrich the same page on recovery — see docs/BRIEFING-FALLBACK.md. **White Glove / Jira handling was removed 2026-09-09** — the WG process was retired; all of it lived in the gitignored skill prompt, archived with restore anchors at docs/disabled/white-glove-jira-briefing.md
├── Views/
...
│   ├── SettingsView.swift                # 7-tab preferences (General, Alerts, Appearance, Checklist, Calendars, Notion, Briefings, Integrations). Appearance folds in the old Display tab; **Briefings** owns the intraday trigger, Claude CLI/skill paths, the Apple Intelligence fallback and Slack/Todoist delivery (moved out of Notion → Calendar Sync, where it was undiscoverable); Notion has a "Notes"/"Calendar Sync" segmented sub-tab (Cal Sync lives here now); Integrations has an "Availability"/"Busy Light"/"Cal.com" segmented sub-tab. Calendars tab is a two-column table: "Monitor" (enabledCalendarIDs) + "→ Notion" (calendarNotionSyncEnabledCalendarIDs)
...
**Availability page** — `AvailabilityPushService` pushes a sanitised free/busy snapshot to Supabase on a timer; a public Vercel/Next.js page (its own repo, [`adamswbrown/availability-page`](https://github.com/adamswbrown/availability-page)) reads it. Full detail: [docs/AVAILABILITY-PAGE.md](docs/AVAILABILITY-PAGE.md).
...
- **Duplicate *prevention* (two layers, added 2026-09-15)** — detection above is after-the-fact; these stop duplicates being written. `existing` is a snapshot taken once at run start, so anything the run itself creates (or another writer creates mid-run) is absent from it, and the CREATE branch would fire twice for one `Apple Event ID`. (1) **Run registry** — `CalendarSyncRunRegistry` (pure value type, unit-tested in `CalendarSyncRunRegistryTests`) records every appleID→pageID the run writes; a repeat PATCHes that page and logs `duplicate appleID within run`. This covers the same event arriving twice in one run, which the **multi-calendar** path makes reachable: `CalendarEventMapper.compositeAppleID` has no calendar component, so one event shared by two opted-in calendars yields the same ID twice. (2) **Pre-create lookup** — `CalendarSyncNotionQueries.findPageID` queries Notion filtered on `Apple Event ID` immediately before `POST /pages`; a hit is adopted and PATCHed, logging `adopted pre-existing row … snapshot was stale`. Measured Notion create→query visibility is ~2s, so this is reliable. Costs one extra query per create only — zero in steady state, where `created=0`. Historical note: five rows of the *Weekly Cadence Dr Migrate UK/Europe* series were duplicated this way on 2026-06-30 (two bursts 106s apart) and cleaned up on 2026-09-15; the originating trigger is unrecoverable (that day's log had rotated away), but both layers close every route to it.
...
| `slackBotToken` | Slack bot token (`chat:write`, `chat:write.public`) used by the Apple Intelligence briefing fallback to post to `#daily-breifings`, and by the headless intraday skill. Editable in Settings → Briefings → Delivery. |
| `todoistApiToken` | Todoist API token used by the briefing fallback to sync action items into the "Daily Briefing" project. Todoist `/rest/v2` is dead (HTTP 410) — the app uses `/api/v1`, whose list responses are wrapped in `{"results": […]}`. |
...
- **CI and release run on `macos-26` with `maxim-lobanov/setup-xcode@v1` `latest-stable`** (Xcode 26.6 / macOS 26.5 SDK). Do NOT pin an old Xcode — the code uses Swift 6.x region-based concurrency that Xcode 15.4 rejects with "reference to captured var 'self'" errors. **Do not drop back to `macos-15` either:** it tops out at Xcode 26.3, whose macOS 26.2 SDK lacks `SystemLanguageModel.tokenCount` / `.contextSize` (macOS 26.4+), so the Apple Intelligence briefing fallback fails to compile there.
