 ...
 *
 * We match any event that *overlaps* the window [now, horizon] — i.e. hasn't
 * ended yet (`end_utc >= now`) and starts before the horizon. This is what
 * lets an in-progress multi-day OOO block (started yesterday, ends Friday)
 * still surface, which a naive `start_utc >= now` filter would drop.
 */
export async function fetchFreeBusy(): Promise<FreeBusyEvent[]> {
  ...
  const params = new URLSearchParams({
    select: "event_id,start_utc,end_utc,is_all_day,is_tentative,is_ooo,status",
